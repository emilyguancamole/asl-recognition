...
def get_default_args():
    ''' Set an argument parser with default arguments for the training script. 
    Names of the arguments defined in the ArgumentParser correspond to the command-line options prefixed with `--` '''
...
def train(args):
    ...
    if args.plot_stats:
        plot_performance(losses, train_accs, val_accs)
    # PLOT 1: Learning rate progress
    if args.plot_lr:
        plot_learning_rate(lr_progress)

    print("\nAny desired statistics have been plotted.\nThe experiment is finished.")
    logging.info("\nAny desired statistics have been plotted.\nThe experiment is finished.")

def plot_performance(losses, train_accs, val_accs):
    fig, ax = plt.subplots()
...
def plot_learning_rate(lr_progress):
